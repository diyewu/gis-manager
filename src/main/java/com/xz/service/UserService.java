...
public class UserService {
	...
	public List<CategoryTreeBean> getTreeList(String roleId) {
		System.out.println("_______roleId="+roleId);
		...
		System.out.println("_______1____________"+list);
		return list;
	}
...
//			devBind.setHref(rs.getString("menu_url"));
	...
	public boolean isExitUser(String userName,String userId){
		...
		if (StringUtils.isBlank(userId)) {
			...
			list = jdbcTemplate.queryForList("select count(1) from user_login where user_name = ? and id <> ? and is_delete = 0 ",userName,userId);
		}
		if (list != null && list.size() > 0) {
